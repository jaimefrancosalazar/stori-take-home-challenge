# ECS Module
# Creates ECS Cluster, Service, Task Definition with Auto Scaling
...
resource "aws_iam_role_policy" "ecs_task_execution_cwlogs" {
  name = "${var.project_name}-ecs-cwlogs-policy"
...
# Auto Scaling Policy - Scale Out (CPU > 60%)
...
# CloudWatch Alarm - High CPU (triggers scale out)
...
# Auto Scaling Policy - Scale In (CPU < 20%)
...
# CloudWatch Alarm - Low CPU (triggers scale in)
